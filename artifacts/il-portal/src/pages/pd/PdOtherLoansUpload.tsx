...
export default function PdOtherLoansUpload() {
  const qc = useQueryClient();
  const [rows, setRows] = useState<Record<string, string>[]>([]);
  const [replace, setReplace] = useState(true);

  const list = useQuery({
    queryKey: ["pd", "other-loans"],
    queryFn: () => apiGet<Row[]>("/pd/other-loans"),
  });

  const mut = useMutation({
    mutationFn: (body: { rows: Record<string, string>[]; replace: boolean }) =>
      apiPost<{ ok: boolean; count: number }>("/pd/other-loans/bulk", body),
    onSuccess: (j) => {
      showToast(`Other loans updated · ${j.count} rows`, "ok");
      setRows([]);
  ...
  function downloadTemplate() {
    downloadCsv(
      ...
      ["Client ID", "Bank", "Loan Type", "Loan Amount", "Outstanding", "EMI", "Tenure Left", "Remarks"],
    );
  }
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      <div>
        <h1 className="text-xl font-extrabold text-slate-800">📑 Other Loans Upload</h1>
        <p className="text-sm text-slate-500">Bureau / NBFC / Bank existing loans bulk upload karein</p>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => setRows([])} className="px-3 py-1.5 text-xs font-bold border border-slate-300 rounded">Cancel</button>
              <button
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5">
        <div className="font-bold text-slate-800 mb-2">Existing Other-Loan Records ({allRows.length})</div>
          ...
          <table className="w-full text-xs">
            <thead className="bg-slate-50 text-[10px] uppercase text-slate-500 sticky top-0">
              <tr>
                ...
                <th className="text-left px-2 py-1.5">Loan Type</th>
                <th className="text-right px-2 py-1.5">Outstanding</th>
                <th className="text-right px-2 py-1.5">EMI</th>
              </tr>
            </thead>
            <tbody>
              {allRows.slice(0, 200).map((r) => (
                <tr key={r.id} className="border-t border-slate-100">
                  ...
                  <td className="px-2 py-1.5">{String(r["Bank"] ?? r["Bank Name"] ?? "")}</td>
                  <td className="px-2 py-1.5">{String(r["Loan Type"] ?? "")}</td>
                  <td className="px-2 py-1.5 text-right">{String(r["Outstanding"] ?? "")}</td>
                  <td className="px-2 py-1.5 text-right">{String(r["EMI"] ?? "")}</td>
                </tr>
              ))}
            </tbody>
          </table>
